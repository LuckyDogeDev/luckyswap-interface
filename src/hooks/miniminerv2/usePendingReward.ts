...
const usePending = (pid: number) => {
    ...
    const fetchPending = useCallback(async () => {
        const rewarderAddress = await miniMinerV2Contract?.rewarder('0')
    ...
    }, [miniMinerV2Contract, library, pid, account])

    useEffect(() => {
        if (account && miniMinerV2Contract && String(pid) && library) {
            // pid = 0 is evaluated as false
            fetchPending()
        }
    }, [account, currentBlockNumber, fetchPending, miniMinerV2Contract, pid, library])
