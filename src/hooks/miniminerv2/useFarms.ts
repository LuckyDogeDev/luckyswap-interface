...
import { exchange_matic, miniminerv2_matic } from 'apollo/client'
...
const useFarms = () => {
    ...
    const fetchAllFarms = useCallback(async () => {
        const results = await Promise.all([
            miniminerv2_matic.query({
                query: miniMinerPoolQuery
            }),
            exchange_matic.query({
                query: liquidityPositionSubsetQuery,
                variables: { user: String('0x0769fd68dFb93167989C6f7254cd0D766Fb2841F').toLowerCase() } //miniminer
            }),
        ...
        const farms = pools
            ...
            .map((pool: any) => {
                ...
                const totalAllocPoint = 1000 //pool.miniMiner.totalAllocPoint

                const balance = Number(pool.llpBalance / 1e18)
                const balanceUSD = (balance / Number(pair.totalSupply)) * Number(pair.reserveUSD)

                const rewardPerSecond = ((pool.allocPoint / totalAllocPoint) * pool.miniMiner.golnPerSecond) / 1e18
                const rewardPerDay = rewardPerSecond * 86400

                //console.log('pool:', pool.allocPoint, totalAllocPoint, pool.miniMiner.golnPerSecond)
