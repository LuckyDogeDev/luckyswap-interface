...
import { ChainId, Currency, WETH } from '@luckyfinance/sdk'
import { alpine } from '@luckyfinance/lucky-data'
import { ethers } from 'ethers'
import { useActiveWeb3React } from 'hooks/useActiveWeb3React'
import { useAllTokens } from 'hooks/Tokens'
import { useBoringHelperContract, useAlpineContract } from 'hooks/useContract'
import { e10, maximum, minimum, ZERO } from 'goldvein/functions/math'
import { rpcToObj } from 'goldvein/functions/utils'
import { toAmount, toShare } from 'goldvein/functions/alpine'
import { accrue, accrueTotalAssetWithFee, easyAmount, getUSDValue, interestAccrue } from 'goldvein/functions/goldvein'
...
export interface GoldVeinContextProps {
    state: State
    dispatch: React.Dispatch<any>
}

type GoldVeinProviderProps = {
    state: State
    dispatch: React.Dispatch<any>
}

export const GoldVeinContext = createContext<{
    state: State
    dispatch: React.Dispatch<any>
}>({
    state: initialState,
...
export function GoldVeinProvider({ children }: { children: JSX.Element }) {
    ...
    const updatePairs = useCallback(
        async function() {
            ...
            if (boringHelperContract && alPineContract) {
                ...
                const pairs = rpcToObj(await boringHelperContract.pollGoldVeinPairs(account, allPairAddresses))
                ...
                // Get balances, alpine info and allowences for the tokens
...
export function useGoldVeinPairs() {
    const context = useContext(GoldVeinContext)
    if (context === undefined) {
        throw new Error('useGoldVeinPairs must be used within a GoldVeinProvider')
    }
    return context.state.pairs
}

export function useGoldVeinPair(address: string) {
    const context = useContext(GoldVeinContext)
    if (context === undefined) {
        throw new Error('useGoldVeinPair must be used within a GoldVeinProvider')
    }
