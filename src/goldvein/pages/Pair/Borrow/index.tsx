import { BorrowCardHeader, Card, GradientDot, Layout } from 'goldvein/components'
import { GoldVeinContext, useGoldVeinPair } from 'goldvein/context'
...
import BorrowGraphic from 'assets/goldvein/borrow-graphic.png'
import { Helmet } from 'react-helmet'
import { GoldVeinCooker } from 'goldvein/entities'
...
export default function BorrowPair({
    ...
    const onUpdateExchangeRate = useCallback(async () => {
        const result = await new GoldVeinCooker(pair, account, library, chainId).updateExchangeRate().cook()
    ...
    return (
        ...
        >
            <Helmet>
                <title>{i18n._(t`Borrow ${pair?.asset?.symbol}-${pair?.collateral?.symbol}`)}| LuckyFinance</title>
            </Helmet>
            <Card
                className="h-full bg-dark-900"
                header={
