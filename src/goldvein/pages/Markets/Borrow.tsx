...
import ListHeaderWithSort from 'goldvein/components/ListHeaderWithSort'
import React from 'react'
import { ZERO } from '../../functions'
import { getCurrency } from 'goldvein/constants'
import { useActiveWeb3React } from 'hooks/useActiveWeb3React'
import { useGoldVeinPairs } from '../../context'
...
export default function BorrowMarkets(): JSX.Element {
    ...
    const fullPairs = useGoldVeinPairs()
    ...
    return (
        ...
        >
            <Helmet>
                <title>{i18n._(t`Borrow`)} | LuckyFinance</title>
            </Helmet>
                                ...
                                {positions.items.map((pair: any) => {
                                    return (
                                        <div key={pair.address}>
                                            <Link
                                                to={'/bento/goldvein/borrow/' + pair.address}
                    ...
                    {pairs.items &&
                        pairs.items.map(pair => {
                            return (
                                <div key={pair.address}>
                                    <Link
                                        to={'/bento/goldvein/borrow/' + String(pair.address).toLowerCase()}
                                        className="block text-high-emphesis"
                                    >
                                        <div className="grid gap-4 grid-cols-4 md:grid-cols-6 lg:grid-cols-7 py-4 px-4 items-center align-center text-sm  rounded bg-dark-800 hover:bg-dark-pink">
