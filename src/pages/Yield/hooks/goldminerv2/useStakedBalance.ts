...
const useStakedBalance = (pid: number, decimals = 18) => {
    ...
    const fetchBalance = useCallback(async () => {
        const getStaked = async (pid: number, owner: string | null | undefined): Promise<BalanceProps> => {
            try {
                const { amount } = await masterMinerV2Contract?.userInfo(pid, owner)
        ...
        setBalance(balance)
    }, [account, decimals, masterMinerV2Contract, pid])

    useEffect(() => {
        if (account && masterMinerV2Contract) {
            fetchBalance()
        }
    }, [account, setBalance, currentBlockNumber, fetchBalance, masterMinerV2Contract])
