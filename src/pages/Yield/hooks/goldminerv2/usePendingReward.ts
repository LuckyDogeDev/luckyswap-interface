...
const usePending = (pid: number) => {
    ...
    const fetchPending = useCallback(async () => {
        const rewarderAddress = await masterMinerV2Contract?.rewarder('0')
    ...
    }, [masterMinerV2Contract, library, pid, account])

    useEffect(() => {
        if (account && masterMinerV2Contract && String(pid) && library) {
            // pid = 0 is evaluated as false
            fetchPending()
        }
    }, [account, currentBlockNumber, fetchPending, masterMinerV2Contract, pid, library])
